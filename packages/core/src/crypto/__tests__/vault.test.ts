...
describe("VaultManager", () => {
  ...
  describe("updateEntry", () => {
    it("should update entry data and increment version", async () => {
      const originalEntry = await VaultManager.createEntry(
        userId,
        testData,
        testKey,
        testCategory,
      ...
      const updatedEntry = await VaultManager.updateEntry(
        originalEntry,
        newData,
        testKey,
        newCategory,
      ...
      const decryptedData = await VaultManager.decryptEntry(
        updatedEntry,
        testKey,
      );
  ...
  describe("decryptEntry", () => {
    ...
    it("should fail with wrong key", async () => {
      ...
      await expect(
        VaultManager.decryptEntry(entry, wrongMasterKey.key),
      ).rejects.toThrow();
    });
  });
  ...
  describe("searchEntries", () => {
    ...
    beforeEach(async () => {
      entries = [
        await VaultManager.createEntry(
          userId,
    ...
    it("should find entries by content", async () => {
      const results = await VaultManager.searchEntries(
        entries,
        "facebook",
        testKey,
      );
  ...
  describe("utility methods", () => {
    ...
    beforeEach(async () => {
      entries = [
        await VaultManager.createEntry(userId, "data1", testKey, "Passwords", [
          "social",
          "work",
        ]),
        await VaultManager.createEntry(userId, "data2", testKey, "Finance", [
          "bank",
        ]),
        await VaultManager.createEntry(userId, "data3", testKey, "Passwords", [
          "personal",
        ]),
        await VaultManager.createEntry(userId, "data4", testKey, "Documents", [
          "legal",
          "work",
        ]),
      ];
    ...
    describe("getEntriesByCategory", () => {
      it("should filter entries by category", () => {
        const passwordEntries = VaultManager.getEntriesByCategory(
          entries,
          "Passwords",
        );
