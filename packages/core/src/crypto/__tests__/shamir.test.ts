...
describe("ShamirSecretSharing", () => {
  const testSecret =
    "This is a test secret that should be split and reconstructed";
  ...
  describe("splitSecret and reconstructSecret", () => {
    ...
    it("should reconstruct secret with more than minimum shares", async () => {
      ...
      const reconstructed2 = ShamirSecretSharing.reconstructSecret(
        shares.slice(0, 4),
      );
    ...
    it("should generate unique shares each time", async () => {
      ...
      const reconstructed1 = ShamirSecretSharing.reconstructSecret(
        shares1.slice(0, 2),
      );
      const reconstructed2 = ShamirSecretSharing.reconstructSecret(
        shares2.slice(0, 2),
    ...
    it("should handle different secret lengths", async () => {
      ...
      const longSecret =
        "This is a much longer secret that contains more characters and should still work correctly with Shamir's Secret Sharing algorithm";
      ...
      const reconstructedShort = ShamirSecretSharing.reconstructSecret(
        shortShares.slice(0, 2),
      );
      const reconstructedLong = ShamirSecretSharing.reconstructSecret(
        longShares.slice(0, 2),
      );
    ...
    it("should handle special characters and unicode", async () => {
      const specialSecret =
        "Secret with special chars: !@#$%^&*()_+-=[]{}|;:,.<>? and unicode: 🔐🗝️💾";
