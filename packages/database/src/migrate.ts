...
async function runMigrations(): Promise<void> {
  try {
    ...
    const createMigrationsTableSQL = readFileSync(join(__dirname, 'schema', 'migrations_table.sql'), 'utf8');
    ...
    for (const migrationFile of MIGRATION_FILES) {
      ...
      const result = await DatabaseConnection.query(`SELECT id FROM migrations WHERE name = $1`, [migrationFile]);
      ...
      await DatabaseConnection.query(`INSERT INTO migrations (name) VALUES ($1)`, [migrationFile]);
