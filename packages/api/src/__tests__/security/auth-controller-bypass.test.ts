...
describe('Auth Controller Security Bypass Tests', () => {
  ...
  describe('Registration Security Tests', () => {
    ...
    it('should reject empty fields', async () => {
      const emptyFieldTests = [
        ...
        { email: 'test@example.com', password: '', confirmPassword: 'password123' },
        { email: 'test@example.com', password: 'password123', confirmPassword: '' },
      ];
    ...
    it('should enforce password confirmation match', async () => {
      ...
      expect(mockResponse.status).toHaveBeenCalledWith(400);
      expect(mockResponse.json).toHaveBeenCalledWith({ error: "Passwords do not match" });
      expect(mockUserService.createUser).not.toHaveBeenCalled();
    });
  ...
  describe('Security Edge Cases', () => {
    ...
    it('should handle very long input strings', async () => {
      ...
      mockUserService.authenticateUser.mockRejectedValue(new Error('Invalid credentials'));
