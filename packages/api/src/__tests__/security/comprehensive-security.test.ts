    ...
    describe('Secure Random Generation', () => {
      ...
      it('should not use predictable Math.random() patterns', () => {
        ...
        for (let i = 1; i < passwords.length; i++) {
          if (passwords[i].charCodeAt(0) === passwords[i-1].charCodeAt(0) + 1) {
            sequentialCount++;
          }
        }
  ...
  describe('Input Sanitization Security', () => {
    ...
    describe('XSS Prevention', () => {
      ...
      it('should prevent prototype pollution', () => {
        ...
        expect(mockRequest.body.constructor).not.toEqual({ prototype: { isAdmin: true } });
      ...
      it('should limit input size to prevent DoS', () => {
        ...
        expect(Object.keys(mockRequest.body.manyKeys).length).toBeLessThanOrEqual(100);
        expect(mockNext).toHaveBeenCalled();
      });
    });
  ...
  describe('Controller Security', () => {
    ...
    describe('Input Type Validation', () => {
      ...
      it('should reject non-string inputs in registration', async () => {
        const invalidInputs = [
          { email: 123, password: 'password123', confirmPassword: 'password123' },
          { email: 'test@example.com', password: null, confirmPassword: 'password123' },
          { email: 'test@example.com', password: 'password123', confirmPassword: undefined },
        ];
